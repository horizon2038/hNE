...
namespace core
{
    ...
    void ppu::write(address target_address, uint8_t data)
    {
        ...
        switch (register_address)
        {
            case 0x0 :
                ctrl         = data;
                temp_address = static_cast<uint16_t>(
                    (temp_address & 0xF3FF) | ((data & 0x03) << 10)
                );
            ...
            case 0x1 :
                ...
                {
                    std::cout
                        << std::format("PPUMASK <= 0x{:02X}", mask) << std::endl;
                }
                break;
            case 0x3 :
                oam_address = data;
    ...
    uint8_t ppu::read_data()
    {
        ...
        vram_address = normalize_vram_address(
            static_cast<uint16_t>(vram_address + increment_step())
        );
        return return_data;
    }
    ...
    void ppu::write_address(uint8_t data)
    {
        ...
        {
            temp_address = static_cast<uint16_t>(
                (temp_address & 0x00FF) | (static_cast<uint16_t>(data & 0x3F) << 8)
            );

            std::cout << std::format(
                "PPUADDR high <= 0x{:02X}, temp_address=0x{:04X}",
                data,
                temp_address
            ) << std::endl;
        }
        ...
        {
            temp_address = static_cast<uint16_t>(
                (temp_address & 0xFF00) | static_cast<uint16_t>(data)
            );
    ...
    void ppu::write_data(uint8_t data)
    {
        ...
        if (should_trace_ppu_palette() && normalized_address >= 0x3F00
            && normalized_address < 0x3F20)
        {
            std::cout << std::format(
                "PPU PALETTE [0x{:04X}] <= 0x{:02X}",
                normalized_address,
                data
            ) << std::endl;
        }
    ...
    void ppu::render_background()
    {
        ...
        auto scroll_x = static_cast<uint16_t>(fine_x_scroll) * 8
                      + static_cast<uint16_t>(coarse_x_scroll);
        ...
        for (uint16_t screen_y = 0; screen_y < SCREEN_HEIGHT; screen_y++)
        {
            ...
            for (uint16_t screen_x = 0; screen_x < SCREEN_WIDTH; screen_x++)
            {
                ...
                auto nt_x_offset
                    = static_cast<uint16_t>((tile_x_world / 32) & 0x01);

                auto nametable_x
                    = static_cast<uint16_t>((base_nt_x + nt_x_offset) & 0x01);
                auto nametable_y
                    = static_cast<uint16_t>((base_nt_y + nt_y_offset) & 0x01);
                auto nametable_index
                    = static_cast<uint16_t>((nametable_y << 1) | nametable_x);
                auto nametable_base = static_cast<uint16_t>(
                    NAMETABLE_BASE + nametable_index * NAMETABLE_SIZE
                );

                auto tile_address = static_cast<uint16_t>(
                    nametable_base + tile_y_local * 32 + tile_x_local
                );
                auto tile_index          = read_vram(tile_address);

                auto pattern_row_address = static_cast<uint16_t>(
                    pattern_base + tile_index * 16 + tile_row_pixel
                );
                auto low_plane = read_vram(pattern_row_address);
                auto high_plane
                    = read_vram(static_cast<uint16_t>(pattern_row_address + 8));

                auto bit_index = static_cast<uint8_t>(7 - tile_col_pixel);
                auto pixel_color_index = static_cast<uint8_t>(
                    ((low_plane >> bit_index) & 0x01)
                    | (((high_plane >> bit_index) & 0x01) << 1)
                );

                auto palette_select = read_attribute_palette_select(
                    nametable_base,
                    tile_x_local,
                    tile_y_local
                );
                auto palette_value = read_background_palette_entry(
                    palette_select,
                    pixel_color_index
                );

                auto pixel_index
                    = static_cast<uint16_t>(screen_y * SCREEN_WIDTH + screen_x);
    ...
    uint8_t ppu::read_attribute_palette_select(
    ...
    ) const
    {
        auto attribute_address = static_cast<uint16_t>(
            nametable_base + NAMETABLE_TILEMAP
            + static_cast<uint16_t>((tile_y / 4) * 8 + (tile_x / 4))
        );
    ...
    uint8_t ppu::read_background_palette_entry(
    ...
    ) const
    {
        ...
        auto palette_address = static_cast<uint16_t>(
            0x3F00 + (palette_select & 0x03) * 4 + (pixel_color_index & 0x03)
        );
    ...
    uint16_t ppu::normalize_vram_address(uint16_t target_address) const
    {
        ...
        {
            auto nametable_offset
                = static_cast<uint16_t>(mirrored_address - NAMETABLE_BASE);
            auto table_index
                = static_cast<uint16_t>(nametable_offset / NAMETABLE_SIZE);
            auto in_table_offset
                = static_cast<uint16_t>(nametable_offset % NAMETABLE_SIZE);
            ...
            return static_cast<uint16_t>(
                NAMETABLE_BASE + physical_table * NAMETABLE_SIZE + in_table_offset
            );
        }
        ...
        {
            auto palette_address = static_cast<uint16_t>(
                0x3F00 + ((mirrored_address - 0x3F00) % 0x20)
            );

            if (palette_address == 0x3F10 || palette_address == 0x3F14
                || palette_address == 0x3F18 || palette_address == 0x3F1C)
            {
