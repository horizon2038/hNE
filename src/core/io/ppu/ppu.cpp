...
namespace core
{
    ...
    void ppu::write(address target_address, uint8_t data)
    {
        ...
        switch (register_address)
        {
            ...
            case 0x1 :
                ...
                {
                    std::cout << std::format("PPUMASK <= 0x{:02X}", mask)
                              << std::endl;
                }
                break;
            case 0x3 :
    ...
    void ppu::update_vblank()
    {
        ...
        // 1 frame = 341 * 262 PPU cycles = 89342 PPU cycles ~= 29780 CPU cycles.
        // VBlank starts at scanline 241.
        constexpr uint32_t CPU_CYCLES_PER_FRAME = 29780;
        constexpr uint32_t VBLANK_START_CPU_CYCLE
            = (241u * 341u) / 3u; // ~=27393
        ...
        if (!frame_rendered && background_dirty
            && cpu_cycles_in_frame
                   >= (VBLANK_START_CPU_CYCLE + RENDER_DELAY_IN_VBLANK))
        {
            render_background();
    ...
    void ppu::write_address(uint8_t data)
    {
        ...
        {
            temp_address = static_cast<uint16_t>(
                (temp_address & 0x7F00) | static_cast<uint16_t>(data)
            );
    ...
    void ppu::write_data(uint8_t data)
    {
        ...
        if (should_trace_ppu_palette()
            && normalized_address >= 0x3F00
    ...
    void ppu::render_background()
    {
        ...
        auto scroll_x = static_cast<uint16_t>(fine_x_scroll);
        ...
        {
            auto world_y
                = static_cast<uint16_t>(scroll_y_value + screen_y);
            ...
            for (uint16_t screen_x = 0; screen_x < SCREEN_WIDTH; screen_x++)
            {
                ...
                auto nametable_index = static_cast<uint16_t>(
                    (nametable_y << 1) | nametable_x
                );
                ...
                auto high_plane = read_vram(
                    static_cast<uint16_t>(pattern_row_address + 8)
                );
                ...
                auto pixel_index = static_cast<uint16_t>(
                    screen_y * SCREEN_WIDTH + screen_x
                );
    ...
    uint16_t ppu::normalize_vram_address(uint16_t target_address) const
    {
        ...
        {
            auto nametable_offset = static_cast<uint16_t>(
                mirrored_address - NAMETABLE_BASE
            );
